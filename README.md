...
In order to run it, you will need a recent (ideally 2018+) version of MATLAB. Using a command prompt, navigate into your MATLAB directory and, if you have Git installed, perform the following two commands to download the pixel-jump and NEAT-MATLAB source code:
```
git clone https://github.com/hmomin/pixel-jump
git clone https://github.com/hmomin/NEAT-MATLAB
```
Alternatively, you can just download the two repositories manually. From there, open MATLAB and navigate to the place where you downloaded these libraries and add them to the MATLAB path, including all subfolders. Now, all you have to do is enter `pixel_jump` in the Command Window and you should see the GUI below pop up:
